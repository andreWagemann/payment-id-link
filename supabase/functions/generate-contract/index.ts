...
Deno.serve(async (req) => {
  ...
  try {
    ...
    page1.drawText(customer.legal_form.toUpperCase(), { x: 462, y: height - 190, size: 9, font });
    ...
    if (customer.postal_code) {
      page1.drawText(customer.postal_code, { x: 260, y: height - 262, size: 9, font });
    }
    if (customer.city) {
      page1.drawText(customer.city, { x: 353, y: height - 262, size: 9, font });
    }
    page1.drawText(customer.country || "DE", { x: 509, y: height - 262, size: 9, font });
    ...
    if (authorizedPersons && authorizedPersons.length > 0) {
      ...
      if (person1.email) {
        page1.drawText(person1.email, { x: 340, y: height - 347, size: 8, font });
      }
    }
    ...
    if (authorizedPersons && authorizedPersons.length > 0) {
      ...
      if (person1.place_of_birth) {
        page1.drawText(person1.place_of_birth, { x: 128, y: height - 450, size: 8, font });
      }
      ...
      if (person1.nationality) {
        page1.drawText(person1.nationality, { x: 458, y: height - 450, size: 8, font });
      }

      // Privatadresse: Straße, PLZ, Stadt, Ländercode
      if (person1.private_street) {
        page1.drawText(person1.private_street, { x: 100, y: height - 485, size: 8, font });
      }
      ...
      // E-Mail
      if (person1.email) {
        page1.drawText(person1.email, { x: 226, y: height - 550, size: 8, font });
      }

      // Ausweisdokument
      page1.drawText("Ausweis", { x: 70, y: height - 525, size: 8, font });

      // Ausweisnummer
      if (person1.id_document_number) {
        page1.drawText(person1.id_document_number, { x: 199, y: height - 525, size: 8, font });
      }
      ...
      if (person1.id_document_issuing_authority) {
        page1.drawText(person1.id_document_issuing_authority, { x: 406, y: height - 525, size: 8, font });
      }

      // Checkbox "Ich handle in eigenem Namen..."
      page1.drawText("X", { x: 70, y: height - 580, size: 8, font });
